...
quickDetect(color1, color2, variation := 10, x1Ratio := 0.0, y1Ratio := 0.0, x2Ratio := 1.0, y2Ratio := 1.0, item := 1, egg := 0) {
    ...
    pingItems := ["Beanstalk Seed", "Ember Lily", "Sugar Apple", "Burning Bud", "Master Sprinkler", "Rare Summer Egg", "Paradise Egg", "Mythical Egg", "Bug Egg", "Bee Egg"]
...
seedItems := ["Carrot Seed", "Strawberry Seed", "Blueberry Seed", "Orange Tulip", "Tomato Seed"
             , "Daffodil Seed", "Watermelon Seed", "Pumpkin Seed", "Apple Seed"
			 , "Bamboo Seed", "Coconut Seed", "Cactus Seed", "Dragon Fruit Seed"
			 , "Mango Seed", "Grape Seed", "Mushroom Seed", "Pepper Seed"
			 , "Cacao Seed", "Beanstalk", "Ember Lily", "Sugar Apple", "Burning Bud"]
...
Gui, Add, Edit, x180 y165 w40 h18 Limit1 vSavedKeybind gUpdateKeybind, %SavedKeybind%
    ...
    Gui, Show, w520 h460, Linux Premium GAG Macro [FREE VERSION]
    ...
    while (started) {
        if (actionQueue.Length()) {
            ...
            if (cycleFinished) {
                WinActivate, % "ahk_id " . firstWindow
                cycleCount++
                SendDiscordMessage(webhookURL, "[__**CYCLE " . cycleCount . " COMPLETED**__]")
                cycleFinished := 0
                if (!MultiInstanceMode) {
